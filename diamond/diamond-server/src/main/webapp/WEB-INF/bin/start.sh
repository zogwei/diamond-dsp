#!/bin/bash
JAVA_HOME="/app/soft/jdk1.7.0_25"
cd `dirname $0`
BIN_DIR=`pwd`
cd ..
DEPLOY_DIR=`pwd`
...
LOGS_FILE=`sed '/log4j.file/!d;s/.*=//' classes/system.properties | tr -d '\r'`
...
nohup $JAVA_HOME/bin/java $JAVA_OPTS $JAVA_MEM_OPTS $JAVA_DEBUG_OPTS $JAVA_JMX_OPTS -classpath $CONF_DIR:$LIB_JARS com.glsx.cloudframework.server.jetty.SpringJettyMain > $STDOUT_FILE 2>&1 &
